...
### A.2 全量运行的方法：三条命令 + 一次合并（跨时间拼接的口径）（2026-09-23 t241 落表）

**为什么分批**：全量 = 13 路由 × 2 模式 = **26 captures**，每条约 28.5s ⇒ **≈12.3 分钟**，而**当前实测**浏览器在**第 7 条路由左右崩**（t235 / t238）⇒ **一次进程跑完的「单张快照」现在拿不到** ✗。**历史反例（如实登记）**：`docs/screenshots/audit-run2/metrics.json` 是 **09-22 22:24 的 26 captures 单次运行** ⇒ **分批不是自然律、是当前唯一可得的办法** ✓；**哪天单次又能跑完，那条路依然合法** —— **因为本节的要点是「读数必须带时间窗」，不是「必须分批」** ✓。

**三条命令 + 一次合并（逐字可抄）**：

```bash
cd panel
node tools/design-audit.mjs --json --routes=home,chat,sessions,board               --modes=dark,light --out=docs/screenshots/audit-run/b1
node tools/design-audit.mjs --json --routes=memory,graph,agents,inbox              --modes=dark,light --out=docs/screenshots/audit-run/b2
node tools/design-audit.mjs --json --routes=task,knowledge,runtimes,stats,settings --modes=dark,light --out=docs/screenshots/audit-run/b3
cd .. && node panel/tools/merge-audit.mjs --in=docs/screenshots/audit-run/b1/metrics.json,docs/screenshots/audit-run/b2/metrics.json,docs/screenshots/audit-run/b3/metrics.json
```

**⚠️ 路径口径的不对称（会绊人的那一步）**：`--out` **按仓库根解析**（工具既有行为），而合并器的 `--in` 走 `fs.statSync` ⇒ **按 CWD 解析** ⇒ 上面那步合并**必须在仓库根执行**、且 `--in` 写 `docs/...`；**在 `panel/` 里跑合并会「文件不存在」** ✗。

**批的划分与规模（t238 时点读数）**：批 1 = 4 路由 = **8 captures ≈232s** · 批 2 = 4 路由 = **8 captures ≈225s** · 批 3 = 5 路由 = **10 captures ≈289s** ⇒ **8 + 8 + 10 = 26** ✓。**划分不必是这一个**（`--routes` 可任意切），**但三批的并集必须恰好覆盖 13 路由 × 2 模式** ✓。

**跨时间拼接的口径（必须随产物一起走）**：合并器的输出**自己会打这一行** ——
`TIME WINDOW: <最早批 mtime> - <最晚批 mtime>  -- this is a CONCATENATION ACROSS TIME, not one snapshot`
⇒ **各批的 capture 是不同时刻的读数** ⇒ 合并出来的「全量」是**跨时间拼接**的 ✓ —— **这是有意的**（单张快照不可得）；**而任何引用这个「全量」的地方都必须把这一行（或它的时间窗）一起写上** ✗，否则「全量」又变回**一句推断**（正是 t235 去掉的那个东西）。

**聚合规则（逐行，跨合并后的全部 captures）**：**任一份 capture FAIL ⇒ 该行 FAIL**（任何地方有缺陷就是缺陷）· 否则**任一份 PASS ⇒ PASS**（某处量到、且没有任何一处红）· 否则 **`not_measured`** ✓。**为什么第一条必须是「任一份红即红」**：**行 52 正是理由** —— 它在 `sessions` 上红、在 `home` 上绿，**合并读数不得让 home 的绿盖住 sessions 的红** ✗（**多数票 / 「最后一份胜」都会盖住它**）。

**t238 的时点全量读数（2026-09-23；引用时必须连时刻一起引）**：`26 distinct (route/mode) of 26 seen` · `checks: 55 pass / 2 fail / 2 not measured` · `failing rows: 52, 56` ✓。**⚠️ 这份读数会过期**：**行 56 正在被 t240 修**（`#task` 的 title 不含 `task`）⇒ **复跑后应当只剩 52** ⇒ **不得把上面这份当作现行结论** ✗；**要现行读数就复跑上面那四条命令** ✓。

**可证伪条件（什么读数说明这条方法被违背了）**：
1. **有人报了一个「全量」读数却没有时间窗**（产物里没有 `TIME WINDOW` 那一行 / 报告里没写各批 mtime）⇒ **违背** ✗；
2. **把某一批的绿当成全量的绿**（只跑了批 1 就报「全量 PASS」）⇒ **违背** ✗；
3. **合并输出的 distinct (route/mode) ≠ 26 却当成全量**（分母不足：漏了一条路由或一个模式）⇒ **违背** ✗；
4. **聚合规则被实现成多数票或「最后一份胜」**：把某一份 capture 的 `pass: false` 手工改成 `true` 再合并 ⇒ **该行必须仍然 FAIL**（否则第 1 条规则没落地）✗；
5. **反向证据**：合并器输出里**没有** `TIME WINDOW` 那一行 ⇒ **口径没随产物走** ⇒ 该「全量」读数**不可引用** ✗。
