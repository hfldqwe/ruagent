...
| 50 | **会话行的名字不得以平台注入块头开头** | `M1` **RED —— 控制跑 26/199** | **0** | **对象** = `/api/v1/sessions` 每行的 `preview` / `title`。**判定式** = 是否**以我们自己发出的注入块头开头**（`[memory context` / `[role — you are` —— **平台产物的字面量，不是对用户内容的猜测** ✓）。**依据 = 裁决**。**⚠️ 它会一直红，直到存量行被清理 —— 那是诚实的红**：本修复**只前向生效**（旧 transcript 无 sentinel ⇒ 索引器 `user_text()` 是 **no-op** ⇒ 旧行永久保留注入名；**即使重新索引也不会好**）⇒ **下一个读者不要以为判据坏了** ✓ |
...
⇒ **判断一条对策是否成立的方法**：问它**改的是产物还是人**。本代 7 条问题里，凡是落成「改产物」的都**生效**了；凡是落成「提醒」的，**都又漂了一次**。
