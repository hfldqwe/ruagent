...
**⚠️ 适用范围订正（2026-09-23 t196）**：原文的适用对象是「**那 36 行无法修复的存量行**」—— **该前提已被 t167 推翻** ✗ ⇒ **本节的适用对象改为**：**真的没有用户原话的会话** ✓（例：`<user_profile>` 记忆蒸馏会话 ✓、**title 与 preview 都为空的行** ✓）—— 它们**仍需诚实占位名**（**不得显示平台文本** ✓）✓。**⚠️ 新增一条显示规则（t196）**：**title 为空时必须有一个诚实且可区分的显示名** ✓ —— **① 若 `preview` 非空 ⇒ 用 `preview` 的派生** ✓；**② 若 `preview` 也空 ⇒ 用「（无内容的会话）· <日期>」** ✓（**不得显示「未命名会话」而不给日期** ✗ —— 用户报过「大量会话显示未命名会话」⇒ **不可区分**是问题本身 ✓）
