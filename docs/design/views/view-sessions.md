...
**⚠️ 对象集硬要求**：判据的对象**必须是我们自己的注入块头字面量**（`[memory context` / `[role — you are`）—— **与 §12 行 50 同一对象集** ✓ **不得另造一个** ✗ · **不得对用户内容做猜测** ✗
...
- **可判定**：① 该行的名字**不得等于注入块头的截断**（不得以 `[memory context` / `[role — you are` 开头 —— **与行 50 同一对象集** ✓）② 该行的名字**必须含日期** ③ **默认视图里不得出现该行**，**且必须存在一个可见入口能显示它**（可见 ∧ 可聚焦 ✓）
