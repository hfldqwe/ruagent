...
**⚠️ 结论订正（2026-09-23 t196）**：原文写「**这 36 行无法被「修复」**」—— **被推翻** ✗。**t167 证明：sentinel 之前写下的转录同样可恢复** ✓ —— 解析循环用**我们自己的具名块头**（`INJECTED_HEADERS`，`STARTS WITH`）**跳过被污染的 user 消息**，取到其后的**真实用户消息** ✓；**整条都没有用户原话时留空**（**不塞平台文本** ✓）。**读数**：`/api/v1/sessions` 的 title-inj / preview-inj **32 → 0** ✓（t167）；**t196 自读 398 行 = 0 / 0** ✓。⇒ **本节的正确表述**：**可恢复的都已恢复；剩下的是「真的没有用户原话」的会话**（如 `<user_profile>` 蒸馏会话 ✓、空 title 行 ✓）—— **它们不是「修不好」，而是「本来就没有用户的话」** ✓
...
**⚠️ 补一条优先级（2026-09-23 t196）**：**t167 落地后，「从用户的第一条消息派生标题」是主路径** ✓（读数：**title 为空 121 → 7（读取侧派生）→ 33/46（无用户原话 ⇒ 留空）** ✓）⇒ **本节的「`title` 以我们自己的块头开头时丢弃、回退到 `preview`」仍是有效的兜底** ✓，但**优先级写明**：**派生 > 丢弃+回退** ✓（**丢弃**是**下限**，**派生**才是目标 ✓）
...
### S10 窄屏行级动作与触摸目标：44px 与行密度的冲突（2026-09-23 裁决：**选 (ii) 窄屏收进菜单**）

**冲突的双方都是真要求** ✓
- **44px 触摸目标**：外部标准（`docs/research/web-interface-guidelines.md:27`）—— t108 实测**改前** `#sessions` **95/97** 个可点元素 <44px、`#chat` **74/76**、`#agents` **36/38** ✗ ⇒ **触摸下确实难按** ✓
- **行密度**：`#sessions` 在 **390** 下一屏约 **3 行** ✗（t108 实测 **40 → 114px**；`520` **40 → 60px**）—— **列表的可扫读性**是同一批用户的核心用途 ✓
- **而「裁掉文字」已被 t94 禁掉** ✓ ⇒ **不能靠压缩解决** ✓

**裁决：选 (ii) —— 窄屏把次要动作收进菜单** ✓
- **为什么这样做（2026-09-23 更正：原写的理由是「密度」，那是错的）**：**44px 是硬要求**（改前 95/97 个元素 <44px 是**真实的触摸缺陷**）✓；而**菜单真正的价值不是密度，是降低误触风险** ✓ —— **390 档下 3 个相邻的 44px 按钮挤在 254px 的行里，其中「归档」（状态变更）与「删除」（破坏性）相邻** ✗ ⇒ **把次要的破坏性动作收进菜单，降低的是误触风险** ✓。**并且菜单确实在一个带内有用**：**3 个控件装不下而 2 个装得下的区间（内容盒 ≈ [302, 358) ⇒ 视口约 438–494px）** ⇒ **那个带内它把 2 行变回 1 行** ✓
- **为什么不选 (i)（接受 114px）**：那等于**用「列表可扫读性」换「触摸可达性」** —— 而两者服务**同一批用户、同一个动作（在列表里找到并操作一条会话）** ⇒ **牺牲任一方都会让这个动作失败** ✓
- **为什么不选 (iii)「裁掉文字」**：**已被 t94 禁掉**（宁可高一点也不要裁字）✓ ⇒ 它不在候选里 ✓
- **⚠️ 前提修正（本代第 ⑧ 类的又一实例：前提本身不成立）**：原 ⑥ 假设「**把行内控件从 3 减到 2 就能恢复行高**」✗ —— **而实测与算术都表明行高由内容宽度驱动、不由控件数驱动**：390 行内容盒 **254px**，**2 控件时最小内容 302px > 254** ✗、**3 控件时 358px > 254** ✗ ⇒ **都必然 2 行 ⇒ 都是 114px**（ui-list 实测改前 3 控件 = **114px** ✓；且 44 + 10(row-gap) + 44 + 16(padding) = **114** 与实测逐位一致 ✓）⇒ **菜单在 390 档达不到「恢复单行」** ✓ **改判据，不改机制** ✓

**可判定形态（窄屏 `≤520` —— 断点与行 51/52 **同源** ✓）**
1. **行内只保留 2 个控件**：**1 个主操作 + 1 个「更多」** ✓（可判定：窄屏下该行**可见的可点元素 ≤ 2 个**）
2. **「更多」的可访问名必须带对象身份**：`aria-label="更多操作（<会话标题>）"` ✓（**不得只叫「更多」** ✗ —— 屏幕阅读器用户必须知道是哪一行的菜单；与 **C2 的「可识别」**同一思路 ✓）
3. **菜单内每个动作仍须 ≥44×44** ✓（**否则等于把缺陷挪进菜单** ✗ —— 可判定：打开菜单后**每个菜单项 ≥44px**）
4. **键盘可达** ✓：**Tab 能进入**「更多」（可聚焦 · `aria-haspopup="menu"` · `aria-expanded` 随开合变化）· **Enter / Space 打开** · **方向键 / Tab 遍历菜单项** · **Esc 关闭并把焦点还给「更多」** ✓（可判定：**全程无鼠标事件**能完成「打开 → 选中一个次要动作 → 关闭」）
5. **桌面端不受影响** ✓：**>520 时行内动作数量与行高与今天逐位相同**（1440 全部不变 ✓ —— 实测 `#chat` 与 `#sessions` 在 1440 均未变 ✓）
6. **行高（可判定，2026-09-23 改）**：**行高必须等于「该行内容集在该宽度下不换行所需的高度」** —— 即 **不得出现「因控件数导致的额外换行」** ✓。**判定法**：在 **390 与 520 两档**分别测「行高」与「该行内容的**单行**所需高度」（后者用 `scrollWidth` vs `clientWidth` 判 ✓）✓。**并保留一个绝对上界作为回归哨**：**520 档 ≤60px** ✓（那里内容装得下 ✓）· **390 档如实记录为 114px = 内容集所决定的最小值 —— 不是缺陷** ✓
7. **溢出仍为 0** ✓（390 / 768 / 1440 ✓）

**⚠️ 44px 是触摸标准，不是装饰** ✓ —— **不得为了让判据变绿而放宽它** ✗（本代已判过）✓
**保留 t72 的裁决不动** ✓（**窄屏保留「标题 + 时间」** —— 时间用于**识别会话**，是有意裁决 ✓）。**若将来要换回密度，条件是什么**：① **把计数 / 时间簇移出行**（需要**新的位置**，属于新的 IA 决策 ✗ 不能顺手做）或 ② **允许标题截断** —— 而 ② **与 t89 / t94 冲突**（**宁可高一点也不要裁字** ⇒ 需要**重新裁决**）✓ ⇒ **在那之前，390 档的 114px 是正确的结果** ✓

**`#chat` 行的判断：同样处理 —— 但**不需要**菜单** ✓
- 事实：`#chat` 行 **35.5 → 44px**（**只有一个动作** ⇒ 代价仅 **+8.5px** ✓ 一屏仍能放多行 ✓）
- ⇒ **保持内联** ✓ —— 把它也收进菜单会**多一次点击而收益为零**（**没有次要动作可收** ✗）⇒ **不加菜单** ✓
- **若将来 `#chat` 行出现 ≥2 个动作**，则**按 S10 的 1–7 条同样处理** ✓
**⚠️ 适用范围订正（2026-09-23 t196）**：原文的适用对象是「**那 36 行无法修复的存量行**」—— **该前提已被 t167 推翻** ✗ ⇒ **本节的适用对象改为**：**真的没有用户原话的会话** ✓（例：`<user_profile>` 记忆蒸馏会话 ✓、**title 与 preview 都为空的行** ✓）—— 它们**仍需诚实占位名**（**不得显示平台文本** ✓）✓。**⚠️ 新增一条显示规则（t196）**：**title 为空时必须有一个诚实且可区分的显示名** ✓ —— **① 若 `preview` 非空 ⇒ 用 `preview` 的派生** ✓；**② 若 `preview` 也空 ⇒ 用「（无内容的会话）· <日期>」** **⚠️ 「title 为空」的精确定义（2026-09-23 t196 补；两个读数曾不一致，原因是它们量的不是同一件事）**：**「为空」= `title == null` ∨ 去空白后为空串** ✓ —— **实测两种形态都存在**：**t167 读到的 33 个 `null`** · **t196 自读（`limit=400`、398 行）的 46 个空串/去空白为空** ✓ ⇒ **同一条显示规则必须覆盖两者** ✗（**只判 `null` 或只判 `''` 都会漏** ✓）。**判定法**：`!title || !title.trim()` ✓ ✓（**不得显示「未命名会话」而不给日期** ✗ —— 用户报过「大量会话显示未命名会话」⇒ **不可区分**是问题本身 ✓）
