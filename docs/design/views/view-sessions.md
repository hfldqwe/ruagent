...
**裁决：选 (ii) —— 窄屏把次要动作收进菜单** ✓
- **为什么这样做**：**44px 是硬要求**（改前 95/97 个元素 <44px 是**真实的触摸缺陷**）· **114px 行高一屏 3 行是真实的可用性损失** ⇒ **两者都不该让位** ⇒ **菜单是唯一能同时满足两条要求的形态** ✓
...
- **代价（如实写）**：**次要动作从「一眼可见」变成「点一次才可见」** ⇒ **多一次点击 + 一次「更多」的认知负担**；**缓解**：**主操作仍在行内**（见下），且**「更多」的可访问名带上对象身份**（见下）✓
...
6. **行高目标（可判定）**：**390 下 `#sessions` 行高 ≤ 60px** ✓ —— **60 不是拍脑袋**：它是 **`520` 档的实测值**（t108：40 → 60px ✓）⇒ **用同一条栏在更宽档位上的实测值作为更窄档位的上界** ✓
7. **溢出仍为 0** ✓（390 / 768 / 1440 ✓）

**⚠️ 44px 是触摸标准，不是装饰** ✓ —— **不得为了让判据变绿而放宽它** ✗（本代已判过）✓
**退路（若实现证明菜单形态不可行）**：**退到 (i) 接受 114px**，并**必须同时写明** ① **这是有意的取舍**（**密度让位于触摸可达性**）② **何时应重新审视**（当且仅当出现**行高 ≤60px 且动作全部 ≥44px 且主操作仍可见**的新形态时 ✓）✓
